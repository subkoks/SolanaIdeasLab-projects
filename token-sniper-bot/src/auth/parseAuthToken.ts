...
export function parseAuthToken(
  ...
  const legacyUserId = legacyPresent ? (decoded as JWTPayload).userId : undefined;
  const legacyWalletAddress = legacyPresent ? (decoded as JWTPayload).walletAddress : undefined;
  const legacySubscriptionTier = legacyPresent
    ? (decoded as JWTPayload).subscriptionTier
    : undefined;
