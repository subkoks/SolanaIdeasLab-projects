...
export function parseAuthToken(
  decoded: JWTPayload | NormalizedJWTPayload,
  issuer?: string,
  audience?: string,
): AuthenticatedUser {
  ...
  const legacyUserId = hasLegacyClaims ? (decoded as JWTPayload).userId : undefined;
  const legacyWalletAddress = hasLegacyClaims ? (decoded as JWTPayload).walletAddress : undefined;
  const legacySubscriptionTier = hasLegacyClaims ? (decoded as JWTPayload).subscriptionTier : undefined;
