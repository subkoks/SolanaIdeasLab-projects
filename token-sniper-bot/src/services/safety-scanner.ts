...
export class SafetyScannerService {
  ...
  constructor(helius: HeliusService, solana: SolanaService, riskScoring: RiskScoringService, db: DatabaseService) {
    this.helius = helius
    this.solana = solana
  ...
  async scanToken(tokenAddress: string, analysisDepth: 'quick' | 'detailed' | 'comprehensive' = 'quick'): Promise<SafetyScanResult> {
    const startTime = Date.now()

    try {
      logger.info(`Starting safety scan for token: ${tokenAddress} (${analysisDepth})`)
      ...
      const redFlags = this.generateRedFlags(contractAnalysis, liquidityAnalysis, distributionAnalysis, socialAnalysis, developerAnalysis)
      const greenFlags = this.generateGreenFlags(contractAnalysis, liquidityAnalysis, distributionAnalysis, socialAnalysis, developerAnalysis)
      const recommendations = this.generateRecommendations(overallScore, redFlags, greenFlags)
      ...
      logger.info(`Safety scan completed for ${tokenAddress}: ${overallScore}/100 (${safetyLevel})`)
      return result
    } catch (error) {
  ...
  private async analyzeContractSafety(tokenAddress: string, metadata: any, depth: string): Promise<SafetyCategory> {
    const factors: any = {}
    let score = 100
    ...
    factors['mint_authority'] = {
      ...
      description: hasMintAuthority ? 'Mint authority can create more tokens' : 'Mint authority is renounced'
    }
    ...
    factors['freeze_authority'] = {
      ...
      description: hasFreezeAuthority ? 'Freeze authority can freeze transfers' : 'Freeze authority is renounced'
    }
    if (!hasFreezeAuthority) score += 10
    else score -= 20

    // Check for suspicious contract patterns (would use actual analysis in production)
    const hasSuspiciousFunctions = await this.checkSuspiciousFunctions(tokenAddress)
    factors['suspicious_functions'] = {
      value: hasSuspiciousFunctions,
      impact: hasSuspiciousFunctions ? -40 : 5,
      description: hasSuspiciousFunctions ? 'Contract contains suspicious functions' : 'No suspicious functions detected'
    }
    ...
    factors['contract_age'] = {
      ...
      description: isRecent ? 'Contract created recently' : 'Contract has existed for some time'
    }
    if (!isRecent) score += 5
    else score -= 15
    ...
    factors['verified_source'] = {
      ...
      description: isVerified ? 'Contract source is verified' : 'Contract source is not verified'
    }
  ...
  private async analyzeLiquiditySafety(tokenAddress: string, metadata: any, depth: string): Promise<SafetyCategory> {
    const factors: any = {}
    let score = 100
    ...
    // Check holder count
    const holders = await this.helius.getTokenHolders(tokenAddress, depth === 'comprehensive' ? 100 : 20)
    const holderCount = holders.length
    factors['holder_count'] = {
      value: holderCount,
      impact: holderCount > 50 ? 10 : holderCount > 10 ? 5 : holderCount > 0 ? -10 : -20,
    ...
    factors['liquidity_pools'] = {
      ...
      description: hasLiquidityPools ? 'Token has liquidity pools' : 'No liquidity pools found'
    }
  ...
  private async analyzeDistributionSafety(tokenAddress: string, metadata: any, depth: string): Promise<SafetyCategory> {
    const factors: any = {}
    let score = 100
    ...
    factors['top_10_holding'] = {
      value: top10Holding,
      impact: top10Holding > 80 ? -30 : top10Holding > 60 ? -20 : top10Holding > 40 ? -10 : top10Holding > 20 ? 5 : 10,
    ...
    factors['single_holder_dominance'] = {
      ...
      description: isSingleHolderDominated ? 'Single holder dominates' : 'Multiple holders'
    }
    if (isSingleHolderDominated) score -= 25
    else score += 5

    // Check distribution diversity (would use actual analysis)
    const distributionDiversity = await this.getDistributionDiversity(tokenAddress)
  ...
  private async analyzeSocialSignals(tokenAddress: string, metadata: any, depth: string): Promise<SafetyCategory> {
    const factors: any = {}
  ...
  private async analyzeDeveloperReputation(tokenAddress: string, metadata: any, depth: string): Promise<SafetyCategory> {
    const factors: any = {}
    let score = 50 // Default to neutral
    ...
    factors['developer_history'] = {
      ...
      description: hasGoodHistory ? 'Good developer history' : 'Poor developer history'
    }
    ...
    factors['rug_pull_history'] = {
      ...
      description: hasRugPullHistory ? 'Has rug pull history' : 'No rug pull history'
    }
  ...
  private determineSafetyLevel(score: number): 'safe' | 'caution' | 'risky' | 'dangerous' {
  ...
  private generateRecommendations(score: number, redFlags: RedFlag[], greenFlags: GreenFlag[]): string[] {
    const recommendations: string[] = []

    if (score < 30) {
      recommendations.push('🚨 EXTREMELY HIGH RISK - This token appears to be a scam')
  ...
  private getRedFlagType(factor: string): 'security' | 'liquidity' | 'distribution' | 'social' | 'technical' {
    if (factor.includes('mint') || factor.includes('freeze') || factor.includes('suspicious')) {
      return 'security'
    }
  ...
  private getRedFlagSeverity(impact: number): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private getGreenFlagType(factor: string): 'security' | 'liquidity' | 'distribution' | 'social' | 'technical' {
    if (factor.includes('mint') || factor.includes('freeze') || factor.includes('verified')) {
      return 'security'
    }
    if (factor.includes('liquidity') || factor.includes('pool')) {
      return 'liquidity'
  ...
  private async checkSuspiciousFunctions(tokenAddress: string): Promise<boolean> {
  ...
  private async getLiquidityDepth(tokenAddress: string): Promise<number> {
  ...
  private async getDistributionDiversity(tokenAddress: string): Promise<number> {
  ...
  private async getCommunityEngagement(tokenAddress: string): Promise<number> {
    // This would measure community engagement
    return 0.5
  }

  private async getDeveloperReputationScore(tokenAddress: string): Promise<number> {
    // This would check developer's history with other tokens
    return 0.5
  }

  private async hasGoodDeveloperHistory(tokenAddress: string): Promise<boolean> {
    // This would check developer's history with other tokens
    return false
  }

  private async hasRugPullHistory(tokenAddress: string): Promise<boolean> {
