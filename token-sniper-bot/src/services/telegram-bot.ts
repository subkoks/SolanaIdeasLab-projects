...
export class TelegramBotService {
  ...
  registerCommands(): void {
    ...
    this.bot.status((ctx) => this.handleStatus(ctx))
...
**Last Update:** ${new Date().toLocaleString()}
  ...
  private async handleAnalyze(ctx: Context): Promise<void> {
    ...
    try {
      const riskScore = await this.riskScoring.analyzeToken(tokenAddress, 'detailed')
      const metadata = await this.helius.getTokenMetadata(tokenAddress)

      const analysisMessage = this.formatAnalysisMessage(tokenAddress, metadata, riskScore)
      await ctx.replyWithMarkdownV2(analysisMessage, {
        ...Markup.inlineKeyboard([
          [Markup.button.callback('🚨 Set Alert', `alert_${tokenAddress}`)],
          [Markup.button.callback('📊 More Details', `details_${tokenAddress}`)],
    ...
    try {
      ...
      await ctx.replyWithMarkdownV2(riskMessage, {
        ...Markup.inlineKeyboard([
          ...
          [Markup.button.callback('📊 Full Analysis', `analyze_${tokenAddress}`)]
        ])
      })
    } catch (error) {
...
Use /upgrade to subscribe now!
    ...
    await ctx.replyWithMarkdownV2(message, {
      ...Markup.inlineKeyboard([
        ...
        [Markup.button.callback('🏢 Upgrade to Enterprise', 'upgrade_enterprise')],
...
Use /analyze for detailed analysis or /alert to set notifications.
  ...
  private formatAnalysisMessage(tokenAddress: string, metadata: any, riskScore: RiskScore): string {
    const emoji = this.getRiskEmoji(riskScore.riskLevel)
...
${riskScore.recommendations.map(rec => `• ${rec}`).join('\n')}
  ...
  private formatRiskMessage(tokenAddress: string, riskScore: RiskScore): string {
    const emoji = this.getRiskEmoji(riskScore.riskLevel)
...
${riskScore.recommendations.join('\n')}
  ...
  async broadcastSafetyAlert(message: string, targetTier: string = 'all'): Promise<any> {
    try {
  ...
  private formatAlertMessage(alert: TelegramAlert): string {
    const emoji = alert.alertType === 'launch' ? '🚀' : 
                  alert.alertType === 'risk' ? '⚠️' :
