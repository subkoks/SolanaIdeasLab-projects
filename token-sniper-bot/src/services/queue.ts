...
const getBackoffOptions = (): Bull.BackoffOptions => ({
  type:
    config.queue.defaultJobOptions.backoff === "fixed"
      ? "fixed"
      : "exponential",
});

const createJobOptions = (
  overrides: Bull.JobOptions = {},
): Bull.JobOptions => ({
  attempts: config.queue.defaultJobOptions.attempts,
  backoff: getBackoffOptions(),
  ...overrides,
});
...
export class QueueService {
  ...
  constructor() {
    this.tokenAnalysisQueue = new Bull(
      "token analysis",
    ...
    this.alertQueue = new Bull("alerts", config.queue.redisUrl, {
      defaultJobOptions: createJobOptions(),
    });

    this.notificationQueue = new Bull("notifications", config.queue.redisUrl, {
      defaultJobOptions: createJobOptions(),
    });

    this.cleanupQueue = new Bull("cleanup", config.queue.redisUrl, {
      defaultJobOptions: createJobOptions(),
    });
  ...
  private setupEventListeners(): void {
    ...
    this.cleanupQueue.on("completed", (job, _result) => {
  ...
  async addTokenAnalysis(
    tokenAddress: string,
    analysisDepth: string = "quick",
  ): Promise<Bull.Job> {
    return this.tokenAnalysisQueue.add(
      "analyze-token",
      {
        tokenAddress,
        analysisDepth,
