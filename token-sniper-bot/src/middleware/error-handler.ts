...
export const errorHandler = (
  ...
  const isDevelopment = config.server.host === 'localhost' || config.development.enableDebugLogs
...
export const rateLimitErrorHandler = (error: any, req: Request, res: Response, next: NextFunction): void => {
  if (error.name === 'TooManyRequests') {
    const response = {
      ...
      retryAfter: error.resetTime ? Math.ceil((error.resetTime - Date.now()) / 1000) : 60,
      timestamp: new Date().toISOString(),
      path: req.url
    }
...
export const externalApiErrorHandler = (error: any, req: Request, res: Response, next: NextFunction): void => {
...
export const createServiceUnavailableError = (message: string = 'Service unavailable'): CustomError => {
