...
const AUTH_MESSAGE_PREFIX = 'SolanaIdeasLab Token Safety Login'
export const MAX_AUTH_WINDOW_MS = 10 * 60 * 1000
const CLOCK_SKEW_MS = 60 * 1000

const readPrefixedValue = (
  line: string | undefined,
  prefix: string,
): string | null => (line?.startsWith(prefix) ? line.slice(prefix.length) : null)
...
const parseAuthMessage = (
  ...
  const issuedAt = Number(readPrefixedValue(lines[3], 'Issued At: '))
  const expiresAt = Number(readPrefixedValue(lines[4], 'Expires At: '))
